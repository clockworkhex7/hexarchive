...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while num_rolls > 0:
        ...
        if dicenum == 1:
            ...
            current_score = current_score + dicenum
...
def boar_brawl(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 2
    if calculate_boar_brawl(player_score, opponent_score) > 1:
        return calculate_boar_brawl(player_score, opponent_score)
    else:
        return 1
...
def calculate_boar_brawl(player_score, opponent_score):
    return 3 * abs(ten_digit(opponent_score) - one_digit(player_score))
    # END PROBLEM 2
...
def is_always_roll(strategy, goal=GOAL):
    ...
    while OS < goal: 
        PS = 0 #player score
        while PS < goal:
            if strategy(PS, OS) == previous:
                previous = strategy(PS, OS)
                PS += 1
            else:
                return False
        OS += 1
...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    """
    # BEGIN PROBLEM 9
    g = make_averaged(roll_dice, samples_count)
    previous = g(1, dice)
    num_rolls = 1
    count = 1
    while num_rolls < 10:
        current = g(num_rolls + 1, dice)
        if previous < current:
            previous = current
            count = num_rolls + 1
        num_rolls += 1
