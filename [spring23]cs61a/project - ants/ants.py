...
class Place:
    ...
    def __init__(self, name, exit=None):
        ...
        >>> place0 = Place('place_0', exit = None)
        >>> place1 = Place('place_1', exit = place0)
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if self.health - amount > 0:
            self.reflective_damage = amount
        else:
            self.reflective_damage = amount + self.damage
        bees = self.place.bees[:]
        for bee in bees:
            bee.reduce_health(self.reflective_damage)
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        while ex:
            if ex.ant:
                if not ex.ant.be_doubled:
                    ex.ant.double()
                    ex.ant.be_doubled = True
                if ex.ant.is_container and ex.ant.ant_contained:
                    if not ex.ant.ant_contained.be_doubled:
                        ex.ant.ant_contained.double()
                        ex.ant.ant_contained.be_doubled = True
