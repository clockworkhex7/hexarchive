class Transaction:
    ...
    def changed(self):
        ...
        if self.before == self.after or self.after < 0:
            return False
        return True
...
class Account:
    ...
    def deposit(self, amount):
        ...
        """
        self.id = self.id + 1
        self.curr = Transaction(self.id, self.balance, self.balance + amount)
        self.transactions = self.transactions + [self.curr]
...
class Server:
    ...
    def send(self, email):
        ...
        recipient_client = self.clients[email.recipient_name]
        recipient_client.inbox.append(email)
...
def make_change(amount, coins):
    ...
    if amount == smallest or amount == 0:
        return [smallest]
    elif amount != 0:
        if not make_change(amount - smallest, rest):
...
class ChangeMachine:
    ...
    def change(self, coin):
        ...
        for item in result:
            self.coins = remove_one(self.coins, item)
        return result
