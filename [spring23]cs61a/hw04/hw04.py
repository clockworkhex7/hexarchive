...
def balanced(m):
    ...
    left_mass = total_mass(end(left(m)))
    right_mass = total_mass(end(right(m)))
    left_arm = length(left(m))
    right_arm = length(right(m))
    torque_l = left_mass * left_arm
    torque_r = right_mass * right_arm
  
    if is_mobile(end(left(m))):       
        result1 = balanced(end(left(m)))
    if not is_mobile(end(left(m))):
        result1 = True
    if is_mobile(end(right(m))):
        result2 = balanced(end(right(m)))
    if not is_mobile(end(right(m))):
        result2 = True

    if torque_l == torque_r and result1 and result2:
        return True
    else:
        return False
