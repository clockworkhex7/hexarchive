...
class VendingMachine:
    ...
    def vend(self):
        ...
        if self.fund < self.price:
            rest = self.price - self.fund
            return f'Please add ${rest} more funds.'
        else:
            p = self.product
            ...
            self.stock -= 1
            if change:
                return f'Here is your {p} and ${change} change.'
            else:
                return f'Here is your {p}.'
...
def store_digits(n):
    ...
    if n < 10:
        return Link(n)
    print('Link(' + repr({store_digits(n // 10)}) + ')')
